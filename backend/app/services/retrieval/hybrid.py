...
def hybrid_search(
    ...
    *,
    ...
    """Ejecuta retrieval híbrido y devuelve hits ordenados por score fusionado."""
        ...
        for cid in ranked_ids:
            ...
            hits.append(
                SearchHit(
                    ...
                    doc_id=cand.doc_id,
                    score=round(fused_scores.get(cid, 0.0), 6),
    ...
    # Solo vector o solo BM25
    if vector_ranked:
        source = vector_ranked[:limit]
    elif bm25_ranked:
        filtered = [(c, s) for c, s in bm25_ranked if matches_filters(c, filters)]
        source = filtered[:limit]
    else:
        return []
